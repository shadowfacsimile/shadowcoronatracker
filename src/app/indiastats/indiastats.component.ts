...
import { IndiaStates, environment } from './../../environments/environment';

@Component({
	selector: 'app-indiastats',
	templateUrl: './indiastats.component.html',
	styleUrls: [ './indiastats.component.css' ]
})
export class IndiastatsComponent implements OnInit {
	...
	private extractStatesFromResponse(data: any) {
		let tempStates = Object.keys(data.indiaStatesDailyList[0]);
	...
	private extractGrowthForAllStates() {
		this.growths = this.coronaIndiaStatsResponse.indiaStatesDailyList
			.filter((e) => e.status === 'Confirmed')
			.map((e) => e.tt);
		this.dates = this.coronaIndiaStatsResponse.indiaStatesDailyList
			.filter((e) => e.status === 'Confirmed')
			.map((e) => e.date);
