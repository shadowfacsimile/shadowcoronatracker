...
export class AlertsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.coronaSummaryStatsResponse) {
      this.coronaSummarySubscription = this.restService.getCoronaSummaryStats().subscribe(data => this.processSummaryData(data));
    }
    if (!this.coronaStatsResponse) {
      this.coronaCountriesStatsSubscription = this.restService.getCoronaCountriesStats().subscribe(data => this.processCountryStatsData(data));
    }
